...
class AbcMultipleChoiceDatasetProcessor(BaseDatasetProcessor):
    data_name = "abc_multiple_choice"

    def __init__(self, dataset_dir: Path, version_name: str) -> None:
        super().__init__(dataset_dir, version_name)
        self.output_info.instruction = (
            "質問と回答の選択肢を入力として受け取り、選択肢から回答を選択してください。なお、回答は選択肢の番号（例：1）でするものとします。 回答となる数値をint型で返し、他には何も含めないことを厳守してください。"
        )
        ...
        with (self.raw_dir / f"{self.data_name}_train.tsv").open(encoding="utf-8") as f_train:
            ...
            for row in reader:
                train_samples.append(
                    Sample(
                        input=f"質問：{row['question']}\n選択肢：1.{row['choice_1']},2.{row['choice_2']},3.{row['choice_3']},4.{row['choice_4']}",
